...
int normalize(s21_decimal *dec1, s21_decimal *dec2) {
    ...
    if (max_scale > 28) return INF;
    if (min_scale < 0) return NEGATIVE_INF;

    // Приводим оба числа к большему scale
    for (int i = 0; i < diff; i++) {
        s21_decimal *target = (scale1 < scale2) ? dec1 : dec2;
        if (s21_mul(*target, (s21_decimal){{10, 0, 0, 0}}, target) != OK) {
            // Если умножение на 10 невозможно, делаем банковское округление
            bank_round((scale1 > scale2) ? dec1 : dec2, 1);
            return normalize(dec1, dec2); // Пробуем снова с уменьшенным scale
        }
